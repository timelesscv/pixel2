                    ...
                    {Object.entries(settings.enabledCountries).map(([country, enabled]) => (
                        enabled && (
                          ...
                          >
                            ...
                            <span className="text-sm leading-none">{country === 'kuwait' ? '🇰🇼' : country === 'saudi' ? '🇸🇦' : country === 'jordan' ? '🇯🇴' : country === 'oman' ? '🇴🇲' : country === 'uae' ? '🇦🇪' : country === 'qatar' ? '🇶🇦' : '🇧🇭'}</span>
                          </button>
                        )
                    ))}
                    <button
          ...
          <div className="shrink-0 mt-4">
            {isOpen && user ? (
              <div className="mb-4 p-4 bg-surface rounded-2xl border border-surfaceElevated relative overflow-hidden group">
                  <div className="absolute -top-8 -right-8 w-20 h-20 bg-pixel/10 rounded-full blur-2xl transition-transform group-hover:scale-125"></div>
