...
import { syncUserJobStatuses } from '../services/queue';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) { fetchUser(session.user.id); fetchTemplates(session.user.id); }
      else { setIsLoading(false); }
    });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session) { fetchUser(session.user.id); fetchTemplates(session.user.id); }
      else { setUser(null); setTemplates([]); setIsLoading(false); }
    });
    return () => subscription.unsubscribe();
  }, []);

  // API Vault Implementation
  ...
  const refreshSyncStatus = async () => { if (user) await syncUserJobStatuses(user.id); };
  ...
  return (
    <AuthContext.Provider value={{
      ...
      deleteTemplate, getAllUsers, addSubscription, terminateUser, promoteSelf, refreshSyncStatus,
      getApiKeys, addApiKey, toggleApiKey, deleteApiKey,
      activeApiKey: process.env.API_KEY || null
    }}>
      {children}
